...
def api_get(path, params=None, retry=3):
    ...
    for attempt in range(retry):
        try:
            ...
            if attempt < retry - 1:
                wait = (attempt + 1) * 3
                print(f"  [재시도 {attempt+1}/{retry}] {e} — {wait}초 대기")
                time.sleep(wait)
            else:
                print(f"  [오류] {e}")
                return None
...
def fetch_space_pages(space_key):
    ...
    params = {
        "limit": 25,
        "expand": "history.createdBy,version,body.view",
        "depth": "all",
...
def fetch_excerpts(space_key):
    """CQL 검색으로 ArtCraft 페이지의 excerpt를 {page_id: excerpt} 딕셔너리로 반환."""
    excerpts = {}
    start = 0
    limit = 50
    cql = (
...
def build_pages_array():
    ...
    for space_key, team in SPACE_TEAM.items():
        ...
        # excerpt 별도 수집 (CQL 검색으로 한번에)
        excerpts = fetch_excerpts(space_key)
        print(f"  → excerpt {len(excerpts)}개 수집 완료 (페이지 수: {len([p for p in all_space_pages if is_artcraft_page(p.get('title',''))])}개 대상)")
        ...
        for page in all_space_pages:
            ...
            # body.view에서 본문 텍스트 추출, 없으면 CQL excerpt 폴백
            body_excerpt = ""
            try:
                raw = page.get("body", {}).get("view", {}).get("value", "")
                if raw:
                    # HTML 태그, 엔티티 제거 후 앞 200자
                    text = re.sub(r'<[^>]+>', ' ', raw)
                    text = re.sub(r'&[a-z]+;', ' ', text)
                    body_excerpt = re.sub(r'\s+', ' ', text).strip()[:200]
            except:
                pass
            summary = body_excerpt or excerpts.get(page_id, "")
