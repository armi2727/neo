...
def fetch_space_pages(space_key):
    ...
    params = {
        "limit": 50,
        "expand": "history.createdBy,version,body.excerpt",
        "depth": "all",
...
def build_pages_array():
    ...
    for space_key, team in SPACE_TEAM.items():
        ...
        print(f"  → excerpt {len(excerpts)}개 수집 완료")
        ...
        for page in all_space_pages:
            ...
            # body.excerpt 우선 사용, 없으면 CQL excerpt 폴백
            body_excerpt = ""
            try:
                raw = page.get("body", {}).get("excerpt", {}).get("value", "")
                body_excerpt = re.sub(r'<[^>]+>', ' ', raw)  # HTML 태그 제거
                body_excerpt = re.sub(r'\s+', ' ', body_excerpt).strip()[:200]
            except:
                pass
