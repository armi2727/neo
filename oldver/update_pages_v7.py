...
def api_get(path, params=None, retry=3):
    ...
    for attempt in range(retry):
        try:
            ...
            if attempt < retry - 1:
                wait = (attempt + 1) * 3
                print(f"  [재시도 {attempt+1}/{retry}] {wait}초 대기...")
                time.sleep(wait)
            else:
                print(f"  [오류] {e}")
                return None

# ─── 1단계: 제목/메타만 빠르게 전체 수집 ───
def fetch_space_pages(space_key):
    pages = []
    path = f"/rest/api/space/{space_key}/content/page"
    params = {
        "limit": 100,
        "expand": "history.createdBy,version",  # body 없이 빠르게
        "depth": "all",
...
# ─── 2단계: ArtCraft 페이지만 본문 일부 수집 ───
def fetch_page_summary(page_id):
    """페이지 본문 앞부분만 가져오기 (export_view = 텍스트 전용, 가벼움)."""
...
def build_pages_array():
    ...
    for space_key, team in SPACE_TEAM.items():
        ...
        print(f"  → 전체 {len(space_pages)}개 중 ArtCraft {len(artcraft_pages)}개 발견")

        # 2단계: ArtCraft 페이지만 본문 수집
        for i, page in enumerate(artcraft_pages):
            page_id = page["id"]
            ...
            # 진행상황 출력 (10개마다)
            if (i + 1) % 10 == 0:
                print(f"  → {i+1}/{len(artcraft_pages)} 처리 완료...")
            time.sleep(0.2)

        print(f"  → {len(artcraft_pages)}개 완료")
